...
public class GatherLeafAction : GAction
{
    ...
    public GatherLeafAction()
    {
        //add preconditions and effects to the respective dictionaries so the planner 
        //knows what to add or remove

        AddPrecondition(nameof(StateKeys.Location), nameof(LocationType.Forest));

        AddEffect(nameof(StateKeys.LeafInv), 1);
    }
    ...
    private void Initialize(GAgent agent)
    {
        ...
        agent.inventory.numLeaf = 1;
        StartCoroutine("Gather");
    }

    private IEnumerator Gather()
    {
        Debug.Log("I have started gathering Leafs!");
