...
public class BuildCampfireAction : GAction
{
    ...
    public BuildCampfireAction()
    {
        //add preconditions and effects to the respective dictionaries so the planner 
        //knows what to add or remove

        AddPrecondition(nameof(StateKeys.Location), nameof(LocationType.Bed));
        AddPrecondition(nameof(StateKeys.WoodInv), 1);

        AddEffect(nameof(StateKeys.OwnsCampfire), true);
        AddEffect(nameof(StateKeys.WoodInv), 0);
    }
    ...
    {
        Debug.Log("I have started building a campfire!");
